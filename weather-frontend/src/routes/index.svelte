<script>
	...
	function getWeather(longitude, latitude) {
		apiUri = `http://localhost:8090/api/weather?longitude=${longitude}&latitude=${latitude}`
		fetch(apiUri)
		  .then(r => r.json())
		  .then(function(data){
			  weather = data
...
<main>
	{#if !dataLoading}
					...
					<ul class="uk-switcher uk-margin-medium-top">
						...
						<li>
							<div class="uk-grid-small  uk-child-width-1-1 uk-text-center slim" uk-grid>
								...
								<div class="uk-text-lead">{weather.daily.summary}</div>
								<ul uk-accordion class="uk-text-left">
								...
								{#each days as dayOfWeek, i}
									<li>
										...
										<div class="uk-accordion-content uk-text-center">
											<p>Expected conditions: {weather.daily.data[i].summary}</p>
										</div>
									</li>
								{/each}
		...
		{/if}
	{:else}
		<p><object title="loading" vdata="loading.svg" type="image/svg+xml" /></p>
	{/if}
